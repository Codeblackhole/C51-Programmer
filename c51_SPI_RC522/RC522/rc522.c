...
u8 SPIReadByte(void)
{
	...
	for (SPICount = 0; SPICount < 8; SPICount++)                  // Prepare to clock in the data to be read
	{
		...
		CLR_SPI_CK; //_nop_();//_nop_();                                         // Raise the clock to clock the data out of the MAX7456
		if(STU_SPI_MISO)
		{
 			SPIData|=0x01;
		}
		SET_SPI_CK;   //_nop_();//_nop_();                                               // Drop the clock ready for the next bit
	}                                                             // and loop back
